...
import (
	...
	"github.com/npillmayer/fontloading/fontregistry"
...
type TypeCasePromise interface {
	TypeCase() (*fontloading.TypeCase, error)
	Descriptor() fontloading.Descriptor // descriptor of typecase to load, before and after
}

type fontLoader struct {
	await func(ctx context.Context) (*fontloading.TypeCase, error)
	desc  fontloading.Descriptor
}

func (loader fontLoader) TypeCase() (*fontloading.TypeCase, error) {
	return loader.await(context.Background())
}

func (loader fontLoader) Descriptor() fontloading.Descriptor {
	return loader.desc
}
...
func ResolveTypeCase(conf schuko.Configuration, pattern string, style xfont.Style, weight xfont.Weight, size float32) TypeCasePromise {
	...
	go func(ch chan<- fontPlusErr) {
		...
		var f *fontloading.ScalableFont
		...
		if fname != "" { // font is packaged embedded font
			...
			if result.err == nil {
				...
				if f, result.err = fontloading.ParseOpenTypeFont(bytez); result.err == nil {
					result.desc.Family = fname
					name = fname
				}
			}
			if f == nil { // cannot process embedded font => seriously compromised installation
				result.err = fmt.Errorf("internal application error - packaged font not readable: %s", fname)
				ch <- result
				close(ch)
				return
			}
		...
		if f == nil { // next try system fonts
			if desc, _ := FindLocalFont(conf, pattern, style, weight); desc.Family != "" {
				f, result.err = fontloading.LoadOpenTypeFont(desc.Path)
			}
		}
			...
			if fiList, result.err = FindGoogleFont(conf, pattern, style, weight); result.err == nil {
				...
				if confidence > fontregistry.LowConfidence {
					...
					if fpath, result.err = CacheGoogleFont(fiList[i], variant); result.err == nil {
						f, result.err = fontloading.LoadOpenTypeFont(fpath)
	...
	loader.await = func(ctx context.Context) (*fontloading.TypeCase, error) {
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
		case r := <-ch:
...
func FindLocalFont(conf schuko.Configuration, pattern string, style xfont.Style, weight xfont.Weight) (
	desc fontloading.Descriptor, variant string) {
	//
