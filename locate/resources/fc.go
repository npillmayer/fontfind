...
import (
	...
	font "github.com/npillmayer/fontloading"
...
func cacheFontConfigList(conf schuko.Configuration, update bool) (string, bool) {
	...
	if _, err := os.Stat(fcListFilename); err == nil {
		...
		if _, err = os.Stat(dir); os.IsNotExist(err) {
			...
			if err != nil {
				err = fmt.Errorf("user configuration path cannot be created: %s", dir)
				return "", false
			}
		}
	}
	...
	if !path.IsAbs(fcpath) {
		err = fmt.Errorf("fontconfig binary fc-list must point to absolute path: %s", fcpath)
		return "", false
	}
	if fi, err := os.Stat(fcpath); err != nil || (fi.Mode().Perm()&0100) == 0 {
		err = fmt.Errorf("fontconfig configuration points to an invalid binary: %s", fcpath)
		return "", false
	}
	...
	if err != nil {
		err = fmt.Errorf("fontconfig output file cannot be created: %s", fcListFilename)
		return "", false
	}
	return fcListFilename, true
}
...
func loadFontConfigList(conf schuko.Configuration) ([]font.Descriptor, bool) {
	...
	if err != nil {
		err = fmt.Errorf("fontconfig font list cannot be opened: %s", fclist)
		return []font.Descriptor{}, false
	}
	defer fc.Close()
