	...
	loadGoogleFontsDir.Do(func() {
      ...
      please refer to https://developers.google.com/fonts/docs/developer_api`)
		...
		if resp.StatusCode != http.StatusOK {
			...
			err = fmt.Errorf("could not get fonts-diretory from Google font service")
			return
		}
		...
		tracer().Infof("transfered list of %d font from Google Fonts service",
...
func FindGoogleFont(conf schuko.Configuration, pattern string, style xfont.Style, weight xfont.Weight) ([]GoogleFontInfo, error) {
	...
	if err != nil {
		return fi, fmt.Errorf("cannot match Google font: invalid font name pattern: %v", err)
	}
...
func listGoogleFonts(list googleFontsList, pattern string) {
	...
	tracer().Infof("%d fonts in list", len(list.Items))
	tracer().Infof("======================================")
